...
>>> print(3)
  \end{lstlisting}
  \uncover<1->{\alert<1> {\tt 3}}
  \begin{lstlisting}[numbers=none, frame=none, xleftmargin=0pt]
>>> x = lambda x: lambda: x * 3
>>> x
  \end{lstlisting}
  \uncover<2->{\alert<2> {\tt <function <lambda> at 0x...>}}
  \begin{lstlisting}[numbers=none, frame=none, xleftmargin=0pt]
>>> x(3)
  \end{lstlisting}
  \uncover<3->{\alert<3> {\tt <function <lambda> at 0x...>}}
  \begin{lstlisting}[numbers=none, frame=none, xleftmargin=0pt]
>>> x(3)(4)
  \end{lstlisting}
  \uncover<4->{\alert<4> {\tt Error}}
  \begin{lstlisting}[numbers=none, frame=none, xleftmargin=0pt]
>>> t = (1, (2, 3), (4, (5), (6, 7)))
>>> t[1][0]
  \end{lstlisting}
  \uncover<5->{\alert<5> {\tt 2}}
  \begin{lstlisting}[numbers=none, frame=none, xleftmargin=0pt]
>>> t[2][1][0]
  \end{lstlisting}
  \uncover<5->{\alert<5> {\tt Error}}
\end{frame}
...
q = 10
  ...
  \begin{column}{0.5\textwidth}
    ...
    \uncover<1->{\alert<1> {\tt b}} \\\vspace{1cm}

    What will be the result of {\tt c(c(q))}? \\
    \uncover<2->{\alert<2> {\tt 2}} \\\vspace{1cm}

    What will be the result of {\tt d(q)}? \\
    \uncover<3->{\alert<3> {\tt Error}} \\\vspace{1cm}

    What will be the value of {\tt q} after calling {\tt b(q)}? \\
    \uncover<4->{\alert<4> {\tt 10}}
  \end{column}
\end{columns}
\end{frame}
...
def f(m):
  ...
  \uncover<1->{\alert<1> {\tt 14}}

  What will {\tt f(1)} return?
  \uncover<2->{\alert<2> {\tt 22}}

  What will {\tt f(11)} return?
  \uncover<3->{\alert<3> {\tt 11}}

  What will {\tt f(-1)} return?
  \uncover<4->{\alert<4> {\tt 14}}
...
>>> hilfinger(aki, you)
  \end{lstlisting}
  \uncover<1->{\alert<1> {\tt 1}}
  \begin{lstlisting}[numbers=none, frame=none, xleftmargin=0pt]
>>> aki()
  \end{lstlisting}
  \uncover<2->{\alert<2> {\tt 1}}
\end{frame}
...
def rest(r):
  ...
  \uncover<1->{Change {\tt double()} to be compatible with this new {\tt
  RList} representation.}
  \uncover<2->{\alert<2>{Nothing needs to be changed.}}
\end{frame}
